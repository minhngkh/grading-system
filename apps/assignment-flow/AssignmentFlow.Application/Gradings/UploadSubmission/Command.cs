...
public class CommandHandler(BlobServiceClient client) : CommandHandler<GradingAggregate, GradingId, Command>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var relevantEntries = archive.Entries
            .Where(e => globalPattern.Match(e.FullName));
        ...
        var submission = Submission.New(
            ...
            CriterionFilesSet(aggregate, blobEntries));
        aggregate.AddSubmission(submission);
    }

    private static HashSet<CriterionFiles> CriterionFilesSet(GradingAggregate aggregate, List<string> blobEntries)
    {
        ...
        foreach (var selector in aggregate.GetCriterionAttachmentsSelectors())
        {
            criteriaFiles.Add(
                CriterionFiles.New(
                    ...
                    [.. blobEntries
                        .Where(blob => selector.Pattern.Match(blob))
