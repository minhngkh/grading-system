...
function App() {
  ...
  const gradeMutation = useMutation({
    mutationFn: async ({ rubricPrompt, fileText }: { rubricPrompt: string; fileText: string }) => {
      const res = await axios.post("/api/grade-demo", { rubricPrompt, fileText });
      return res.data;
    },
    onSuccess: (data) => {
      if (data.success) {
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block font-medium mb-1">Rubric</label>
              <Textarea
                value={rubric}
